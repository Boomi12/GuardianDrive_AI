  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Redirect to vehicle setup
        navigate('/vehicle-setup');
      }
    } catch (err) {
      setSubmitError(err.error || err.message || 'Registration failed. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[80vh] flex items-center justify-center py-10 animate-fade-in">
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-md border border-purple-500/20 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-slate-400 mt-1">Get started with GuardianDrive AI</p>
        </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full bg-slate-950 border rounded-xl py-2.5 pl-10 pr-4 text-xs text-white placeholder-slate-650 focus:outline-none transition-colors ${
                  errors.name ? 'border-red-500/50' : 'border-slate-800 focus:border-purple-500/80'
                }`}
              />
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="••••••••"
                className={`w-full bg-slate-950 border rounded-xl py-2.5 pl-10 pr-4 text-xs text-white placeholder-slate-650 focus:outline-none transition-colors ${
                  errors.confirmPassword ? 'border-red-500/50' : 'border-slate-800 focus:border-purple-500/80'
                }`}
              />
            </div>
            {errors.confirmPassword && (
              <span className="text-[10px] text-red-400 mt-1 block">{errors.confirmPassword}</span>
            )}
          </div>
